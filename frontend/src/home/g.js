...
d3Graph.init_graph = function (el, props, state) {
    ...
    let choseX = d3.randomUniform(props.width / 4, 3 * props.width / 4);
...
d3Graph.ticked = function(links, nodes, props) {
    nodes
        .attr("x", function(d) { return d.x = Math.max(props.radius, Math.min(props.width - props.radius * 3, d.x)); })
...
d3Graph.update = function(el, props, state) {
    ...
    this.SIMULATION
        .nodes(this.NODE_DATA)
        .on("tick", () => this.ticked(all_links, all_nodes, props));
